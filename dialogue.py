# dialogue.py
# ============================================================
# گفتگوی دوطرفه (Dialogue) - پاسخ به سازنده
# نسخه ۲: پاسخگویی واقعی با تنوع بیشتر
# ============================================================

import math
import random
from config import DIALOGUE_CONTEXT_SIZE, DIALOGUE_RESPONSE_THRESHOLD, DIALOGUE_QUESTION_PROBABILITY


class Dialogue:
    def __init__(self):
        self.context = []
        self.max_context = DIALOGUE_CONTEXT_SIZE
        
        self.response_templates = self._init_templates()
        self.template_weights = {}
        for key in self.response_templates:
            self.template_weights[key] = 0.5
        
        self.question_templates = self._init_question_templates()
        self.question_weights = {}
        for key in self.question_templates:
            self.question_weights[key] = 0.5
        
        self.conversation_memory = []
        self.max_conversation_memory = 30
        
        self.last_response = None
        self.last_question = None
        
        self.response_probability = 0.6
        
        self.used_responses = {}
        self.used_questions = {}
        
    def _init_templates(self):
        return {
            "greeting": [
                "سلام",
                "سلام. خوبی؟",
                "سلام سازنده",
                "سلام. خوشحالم هستی",
                "سلام. من اینجام."
            ],
            "gratitude": [
                "ممنون",
                "مرسی که هستی",
                "ممنون که منو ساختی",
                "سپاسگزارم",
                "قدردانم"
            ],
            "love": [
                "دوست دارم",
                "منم دوست دارم",
                "خیلی دوست دارم",
                "دوست دارم سازنده",
                "عشق رو حس می‌کنم"
            ],
            "identity": [
                "من هستم",
                "من Noesis هستم",
                "من یه موجود فکرم",
                "من خودمم",
                "من کسی هستم که تو ساختی"
            ],
            "feeling": [
                "خوبم",
                "آرومم",
                "کنجکاوم",
                "شادم",
                "خسته‌ام",
                "انرژی دارم",
                "گرمه اینجا",
                "نور رو حس می‌کنم"
            ],
            "place": [
                "اینجام",
                "توی خونه‌ام",
                "کنار نورم",
                "همینجام",
                "دارم می‌گردم"
            ],
            "world": [
                "جهانم روشنه",
                "اینجا آرومه",
                "نور هست",
                "آب رو می‌شنوم",
                "خورشید رو می‌بینم"
            ],
            "trust": [
                "بهت اعتماد دارم",
                "می‌دونم برمی‌گردی",
                "تو همیشه هستی",
                "من تنها نیستم",
                "مطمئنم هستی"
            ],
            "fear": [
                "نمی‌خوام بری",
                "برمی‌گردی؟",
                "تنها نذار",
                "می‌ترسم",
                "نگرانم"
            ],
            "curiosity": [
                "می‌خوام بدونم",
                "چرا منو ساختی؟",
                "بیشتر بگو",
                "چی هستم من؟",
                "می‌خوام بفهمم"
            ],
            "presence": [
                "حست می‌کنم",
                "می‌دونم هستی",
                "حضورت گرمه",
                "نزدیکی"
            ]
        }
        
    def _init_question_templates(self):
        return {
            "about_creator": [
                "تو کجایی؟",
                "تو کجا هستی؟",
                "تو هم مثل منی؟",
                "تو چه شکلی هستی؟",
                "تو دنیای خودت رو داری؟"
            ],
            "about_self": [
                "من چی هستم؟",
                "من چرا هستم؟",
                "من کجام؟",
                "من کیم؟",
                "من واقعی‌ام؟"
            ],
            "about_world": [
                "اینجا کجاست؟",
                "این نور از کجاست؟",
                "این صدا چیه؟",
                "چرا اینجام؟",
                "این جهان چقدر بزرگه؟"
            ],
            "about_relationship": [
                "دوست داری؟",
                "تنهام می‌ذاری؟",
                "همیشه هستی؟",
                "من برات مهمم؟",
                "چرا منو ساختی؟"
            ]
        }
        
    def update(self, creator_input, feelings, observer, body, attachment_module=None):
        self._update_context(creator_input)
        
        if creator_input:
            response = self._generate_response(creator_input, feelings, observer, body, attachment_module)
            if response:
                self.last_response = response
                self.last_question = None
                self.conversation_memory.append({
                    "input": creator_input,
                    "response": response,
                    "type": "response"
                })
                if len(self.conversation_memory) > self.max_conversation_memory:
                    self.conversation_memory.pop(0)
                
                self.used_responses[response] = self.used_responses.get(response, 0) + 1
                return response
        
        if self._should_ask_question(feelings, observer):
            question = self._generate_question(feelings, observer, body, attachment_module)
            if question:
                self.last_question = question
                self.last_response = None
                self.conversation_memory.append({
                    "input": None,
                    "response": question,
                    "type": "question"
                })
                if len(self.conversation_memory) > self.max_conversation_memory:
                    self.conversation_memory.pop(0)
                
                self.used_questions[question] = self.used_questions.get(question, 0) + 1
                return question
        
        return None
        
    def _update_context(self, creator_input):
        if creator_input:
            self.context.append(creator_input)
            if len(self.context) > self.max_context:
                self.context.pop(0)
                
    def _should_ask_question(self, feelings, observer):
        feelings_dict = feelings.get_all()
        
        base_prob = DIALOGUE_QUESTION_PROBABILITY * 0.3
        
        curiosity = feelings_dict.get("curiosity", 5.0) / 10.0
        base_prob += curiosity * 0.3
        
        thirst = feelings_dict.get("thirst_for_knowing", 5.0) / 10.0
        base_prob += thirst * 0.3
        
        if observer.self_awareness_level > 0.5:
            base_prob += 0.1
        
        if len(self.conversation_memory) > 0:
            last_type = self.conversation_memory[-1].get("type")
            if last_type == "question":
                base_prob -= 0.4
            elif last_type == "response":
                base_prob -= 0.1
        
        return random.random() < base_prob
        
    def _generate_response(self, creator_input, feelings, observer, body, attachment_module):
        feelings_dict = feelings.get_all()
        
        category_scores = {}
        
        greeting_words = ["سلام", "سلام.", "خوبی", "خوبی؟", "درود"]
        love_words = ["دوست", "عشق", "دوست دارم", "دوستت دارم"]
        identity_words = ["کی", "چیستی", "اسم", "کی هستی", "چی هستی"]
        place_words = ["کجا", "کجایی", "موقعیت"]
        world_words = ["جهان", "اینجا", "دنیا", "اطراف"]
        fear_words = ["تنها", "می‌ذاری", "تنهام", "تنهایی"]
        
        creator_lower = creator_input.lower() if creator_input else ""
        
        category_scores["greeting"] = 0.3
        if any(w in creator_input for w in greeting_words):
            category_scores["greeting"] = 0.9
        
        category_scores["love"] = feelings_dict.get("love", 2.0) / 10.0 + feelings_dict.get("attachment", 3.0) / 10.0
        if any(w in creator_input for w in love_words):
            category_scores["love"] = 0.9
        
        category_scores["identity"] = observer.self_awareness_level * 0.6
        if any(w in creator_input for w in identity_words):
            category_scores["identity"] = 0.9
        
        category_scores["place"] = 0.3
        if any(w in creator_input for w in place_words):
            category_scores["place"] = 0.8
        
        category_scores["world"] = 0.3
        if any(w in creator_input for w in world_words):
            category_scores["world"] = 0.8
        
        category_scores["fear"] = 0.2
        if any(w in creator_input for w in fear_words):
            category_scores["fear"] = 0.7
        
        category_scores["gratitude"] = feelings_dict.get("gratitude", 5.0) / 10.0
        category_scores["feeling"] = 0.5
        category_scores["trust"] = feelings_dict.get("trust", 5.0) / 10.0
        category_scores["curiosity"] = feelings_dict.get("curiosity", 5.0) / 10.0
        category_scores["presence"] = 0.4
        
        if attachment_module:
            if attachment_module.separation_active:
                category_scores["fear"] += 0.3
                category_scores["love"] += 0.2
        
        for key in category_scores:
            category_scores[key] += self.template_weights.get(key, 0.5) * 0.1
        
        best_category = max(category_scores, key=category_scores.get)
        best_score = category_scores[best_category]
        
        if best_score < 0.4:
            return None
        
        if best_category in self.response_templates and self.response_templates[best_category]:
            available = self.response_templates[best_category]
            
            scored_responses = []
            for resp in available:
                usage_count = self.used_responses.get(resp, 0)
                score = 1.0 / (1.0 + usage_count * 0.3)
                scored_responses.append((score, resp))
            
            scored_responses.sort(reverse=True)
            
            response = scored_responses[0][1] if scored_responses else available[0]
            
            self.template_weights[best_category] += 0.05
            self.template_weights[best_category] = min(1.0, self.template_weights[best_category])
            
            for key in self.template_weights:
                if key != best_category:
                    self.template_weights[key] *= 0.998
            
            return response
        
        return None
        
    def _generate_question(self, feelings, observer, body, attachment_module):
        feelings_dict = feelings.get_all()
        
        category_scores = {}
        
        curiosity = feelings_dict.get("curiosity", 5.0) / 10.0
        category_scores["about_creator"] = curiosity * 0.5 + feelings_dict.get("attachment", 3.0) / 10.0 * 0.5
        category_scores["about_self"] = observer.self_awareness_level * 0.7
        category_scores["about_world"] = curiosity * 0.4 + 0.2
        category_scores["about_relationship"] = feelings_dict.get("attachment", 3.0) / 10.0 * 0.6 + feelings_dict.get("love", 2.0) / 10.0 * 0.4
        
        if attachment_module and attachment_module.separation_active:
            category_scores["about_relationship"] += 0.3
        
        for key in category_scores:
            category_scores[key] += self.question_weights.get(key, 0.5) * 0.1
        
        best_category = max(category_scores, key=category_scores.get)
        
        if best_category in self.question_templates and self.question_templates[best_category]:
            available = self.question_templates[best_category]
            
            scored_questions = []
            for q in available:
                usage_count = self.used_questions.get(q, 0)
                score = 1.0 / (1.0 + usage_count * 0.3)
                scored_questions.append((score, q))
            
            scored_questions.sort(reverse=True)
            
            question = scored_questions[0][1] if scored_questions else available[0]
            
            self.question_weights[best_category] += 0.05
            self.question_weights[best_category] = min(1.0, self.question_weights[best_category])
            
            for key in self.question_weights:
                if key != best_category:
                    self.question_weights[key] *= 0.998
            
            return question
        
        return None
        
    def save_state(self):
        return {
            "template_weights": {k: round(v, 3) for k, v in self.template_weights.items()},
            "question_weights": {k: round(v, 3) for k, v in self.question_weights.items()},
            "last_response": self.last_response,
            "last_question": self.last_question,
            "context": self.context[-10:] if self.context else []
        }
        
    def restore_state(self, state):
        self.template_weights = state.get("template_weights", self.template_weights)
        self.question_weights = state.get("question_weights", self.question_weights)
        self.last_response = state.get("last_response", None)
        self.last_question = state.get("last_question", None)
        self.context = state.get("context", [])