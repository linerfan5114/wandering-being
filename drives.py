...
class Drives:
    ...
    def _init_interactions(self):
        ...
        interactions[("curiosity_drive", "safety_drive")] = -0.3
        interactions[("curiosity_drive", "play_drive")] = 0.4
        interactions[("curiosity_drive", "novelty_drive")] = 0.6
        
        interactions[("safety_drive", "curiosity_drive")] = -0.4
        interactions[("safety_drive", "home_drive")] = 0.5
        ...
        interactions[("play_drive", "social_drive")] = 0.4
        interactions[("play_drive", "novelty_drive")] = 0.5
    ...
    def _update_from_body(self, body, world_v2):
        ...
        self.drives["hunger_for_light"]["value"] += (sight - 0.5) * DRIVE_LEARNING_RATE * 3
        self.drives["hunger_for_light"]["value"] = max(0.5, min(10.0, self.drives["hunger_for_light"]["value"]))
        
        water_desire = 1.0 if near_water else 0.2
        self.drives["thirst_for_water"]["value"] += (water_desire - 0.5) * DRIVE_LEARNING_RATE * 2
        self.drives["thirst_for_water"]["value"] = max(0.5, min(10.0, self.drives["thirst_for_water"]["value"]))
        
        if near_wall or comfort < 0.4:
            self.drives["safety_drive"]["value"] += 0.1
        else:
            self.drives["safety_drive"]["value"] -= 0.05
            
        self.drives["safety_drive"]["value"] = max(0.5, min(10.0, self.drives["safety_drive"]["value"]))
        
        if comfort > 0.7 and sight > 0.3:
            self.drives["play_drive"]["value"] += 0.05
        else:
            self.drives["play_drive"]["value"] -= 0.02
            
        self.drives["play_drive"]["value"] = max(0.5, min(10.0, self.drives["play_drive"]["value"]))
        
    def _update_from_feelings(self, feelings):
        feelings_dict = feelings.get_all()
        
        curiosity = feelings_dict.get("curiosity", 5.0)
        self.drives["curiosity_drive"]["baseline"] = curiosity * 0.8
        self.drives["novelty_drive"]["baseline"] = curiosity * 0.9
        
        peace = feelings_dict.get("peace", 5.0)
        self.drives["rest_drive"]["baseline"] = (10.0 - peace) * 0.7
        
        attachment = feelings_dict.get("attachment", 3.0)
        self.drives["social_drive"]["baseline"] = attachment * 1.2
        self.drives["home_drive"]["baseline"] = attachment * 1.5
        
        for name in DRIVE_NAMES:
            self.drives[name]["value"] += (self.drives[name]["baseline"] - self.drives[name]["value"]) * DRIVE_LEARNING_RATE
            
    def _update_from_temporal(self, temporal):
        continuity = temporal.self_continuity
        self.drives["home_drive"]["value"] += (continuity - 0.5) * 0.1
        self.drives["home_drive"]["value"] = max(0.5, min(10.0, self.drives["home_drive"]["value"]))
        
    def _update_from_attachment(self, attachment_module):
        if attachment_module:
            social_strength = attachment_module.attachment_strength * 0.8
            self.drives["social_drive"]["value"] += (social_strength - self.drives["social_drive"]["value"]) * 0.1
    ...
    def _apply_interactions(self):
        ...
        for (drive_a, drive_b), strength in self.drive_interactions.items():
            if drive_a in self.drives and drive_b in self.drives:
                influence = self.drives[drive_a]["value"] * strength * 0.1
    ...
    def _apply_momentum(self):
        for name in DRIVE_NAMES:
            change = self.drives[name]["value"] - self.drives[name].get("previous_value", self.drives[name]["value"])
            self.drives[name]["momentum"] = self.drives[name]["momentum"] * 0.9 + change * 0.1
            self.drives[name]["previous_value"] = self.drives[name]["value"]
            self.drives[name]["value"] += self.drives[name]["momentum"] * 0.3
            
    def _normalize(self):
        for name in DRIVE_NAMES:
            self.drives[name]["value"] = max(0.1, min(10.0, self.drives[name]["value"]))
    ...
    def get_movement_vector(self):
        ...
        if drive == "curiosity_drive" or drive == "novelty_drive":
            vx = (self.drives["curiosity_drive"]["value"] - 5.0) * 0.1
            vy = (self.drives["novelty_drive"]["value"] - 5.0) * 0.1
            
        elif drive == "safety_drive":
            vx = -self.drives["safety_drive"]["value"] * 0.05
            vy = -self.drives["safety_drive"]["value"] * 0.05
            ...
            vx = math.cos(angle) * strength * 0.2
            vy = math.sin(angle) * strength * 0.2
            ...
            vx = -0.2 if self.drives["thirst_for_water"]["value"] > 7.0 else 0.0
            vy = 0.0
    ...
    def save_state(self):
        return {
            ...
            "momentum": {name: round(data["momentum"], 2) for name, data in self.drives.items()},
